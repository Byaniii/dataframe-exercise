...
df["Discount"] = np.where(df["Quantity"] >= 5, 0.10, 0)

# Step 2: Add FinalTotal column (Total - discount)
df["FinalTotal"] = df["Quantity"] * df ["Price"]
df['FinalTotal'] = df ['Total'] * (1 - df['Discount'])

# Step 3: Show selected columns
print(df[["OrderID", "Product", "Total", "Discount", "FinalTotal", "Quantity"]]) 
...
# Recalculate Total and FinalTotal after discount
df["Total"] = df["Quantity"] * df["Price"]
df["Discount"] = np.where(df["Quantity"] >= 5, 0.10, 0)
df["FinalTotal"] = df["Total"] * (1 - df["Discount"])

# Show last few rows including new record
print(df.tail(3))
